Arena = function(game,props) {
    ...
    var columns = [];
    var numberColumnI = 50;
    var sizeArena = 100*boxArena.scaling.x -50;
    //var ratio = ((100/numberColumn)/100) * sizeArena;
    for (var i = 0; i <= numberColumnI; i++) {
		for (var j = 0; j <= 50; j++) {
			columns[i] = [];
			let mainCylinder = BABYLON.Mesh.CreateCylinder("cyl0-"+i, 100, 5, 5, 20, 4, scene);
			mainCylinder.position = new BABYLON.Vector3(-sizeArena/numberColumnI *i, 30/2, -50 + j*50);
			mainCylinder.material = materialWall;
			columns[i].push(mainCylinder);
			mainCylinder.checkCollisions = true;
		}
	}
	var materialWall1= new BABYLON.StandardMaterial('materialWall1', scene);
	materialWall1.diffuseColor=new BABYLON.Color3((59/255), (195/255), (203/255));
	for (var i = 0; i <= numberColumnI; i++) {
		for (var j = 0; j <= 10; j++) {
			columns[i] = [];
			let mainCylinder = BABYLON.Mesh.CreateCylinder("cyl0-"+i, 100, 5, 5, 20, 4, scene);
			mainCylinder.position = new BABYLON.Vector3(-sizeArena/numberColumnI *i, 30/2, -50 + -j*50);
			mainCylinder.material = materialWall1;
			columns[i].push(mainCylinder);
			mainCylinder.checkCollisions = true;
		}
	}

	var materialWall2= new BABYLON.StandardMaterial('materialWall2', scene);
	materialWall2.diffuseColor=new BABYLON.Color3((255), (195/255), (203/255));
	for (var i = 0; i <= numberColumnI*2; i++) {
		for (var j = 0; j <= 10; j++) {
			columns[i] = [];
			let mainCylinder = BABYLON.Mesh.CreateCylinder("cyl0-"+i, 100, 5, 5, 20, 4, scene);
			mainCylinder.position = new BABYLON.Vector3(-sizeArena/numberColumnI *i, 30/2, -50 + -j*100);
			mainCylinder.material = materialWall2;
			columns[i].push(mainCylinder);
			mainCylinder.checkCollisions = true;
		}
	}
	...
	var numberblocks = 20;
	var sizeArena = 100*boxArena.scaling.x -50;
	var ratio = ((100/numberblocks)/100) * sizeArena;
	for (var i = 1000; i <= numberblocks+1000; i++) {
		columns[i] = [];
			let mainCylinder = BABYLON.Mesh.CreateBox("box0-"+i, 0.3, scene);
			let max=sizeArena;
			let min=-sizeArena;
		for (let y = 1; y <= numberblocks - 1; y++) {
			mainCylinder.position = new BABYLON.Vector3(-(sizeArena / 2) + (ratio * y), 30 / 2, Math.floor(Math.random() * (50 - 1 +1)) + 1 * 2);
		}
			mainCylinder.material = materialWall;
		columns[i].push(mainCylinder);
			mainCylinder.checkCollisions = true;
        ...
        /*if(numberColumn>0){
            columns[i] = [];
            let mainCylinder = BABYLON.Mesh.CreateCylinder("cyl0-"+i, 100, 5, 5, 20, 4, scene);
            mainCylinder.position = new BABYLON.Vector3(-sizeArena/i, 30/2, -20 + (50 * i));
            mainCylinder.material = materialWall;
            columns[i].push(mainCylinder); 
            mainCylinder.checkCollisions = true;
            ...
            if(numberColumn>1){
                for (let y = 1; y <= numberColumn - 1; y++) {
                    ...
                    newCylinder.position = new BABYLON.Vector3(-(sizeArena/2) + (ratio*y),30/2,columns[i][0].position.z*2);
